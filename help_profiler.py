    ...
    def _load(self) -> None:
        for row in _read_csv(self.lexical_metrics_path):
            ...
            self.lexical[key] = {
                ...
                "orthographic_neighborhood_density": _float(row.get("orthographic_neighborhood_density")),
...
def flashcard_observations(memory: dict[str, Any]) -> list[dict[str, Any]]:
    items = {str(item.get("id") or ""): item for item in memory.get("items", []) if isinstance(item, dict)}
    events = [event for event in memory.get("events", []) if isinstance(event, dict)]
    if not events:
        events = [event for item in items.values() for event in item.get("events", []) if isinstance(event, dict)]
    result = []
    seen: set[tuple[Any, ...]] = set()
    for index, event in enumerate(events, 1):
        item = items.get(str(event.get("item_id") or ""), {})
        outcome = {"correct": "correct", "partial": "partial", "miss": "incorrect"}.get(str(event.get("result") or ""))
        if not outcome:
            continue
        timestamp = str(event.get("label") or event.get("at") or "")
        session_ref = str((event.get("eeg_annotation") or {}).get("jsonl") or str(event.get("label") or "")[:10] or "flashcard-local")
        ...
        result.append(_observation(
            ...
            word=str(event.get("front") or item.get("raw_front") or item.get("term") or event.get("term") or ""),
            outcome=outcome,
...
def conjugation_observations(*session_dirs: Path | None) -> list[dict[str, Any]]:
    ...
    for path in sorted(set(paths)):
        ...
        for index, line in enumerate(lines, 1):
            ...
            source_ref = str(row.get("behavioral_event_id") or event.get("event_id") or f"{path.name}:{index}")
            if source_ref in seen:
                continue
            seen.add(source_ref)
            expected = str(event.get("expected_phrase") or event.get("expected") or "")
            speech = event.get("speech_recognition") if isinstance(event.get("speech_recognition"), dict) else {}
            result.append(_observation(
    ...
    for path in sorted(set(paths)):
        ...
        for index, line in enumerate(lines, 1):
            ...
            if source_ref in seen:
                continue
            seen.add(source_ref)
            word = str(event.get("infinitive") or event.get("phrase") or event.get("expected") or "")
            result.append(_observation(
                source=event_type,
                source_ref=source_ref,
                session_ref=str(row.get("behavioral_session_id") or path.stem),
...
def mlf_observations(db_path: Path) -> list[dict[str, Any]]:
    ...
    for event_id, event_type, timestamp, _order, session_id, unit_id, raw_payload in rows:
        ...
        if event_type == "trial.start":
            starts[trial_id] = {"payload": payload, "unit_id": unit_id, "session_id": session_id, "timestamp": timestamp}
    ...
    for trial_id, candidates in score_events.items():
        ...
        for candidate in candidates:
            ...
            review_status = str((payload.get("score_metadata") or {}).get("human_review_status") or "machine_confirmed")
            ...
            is_correction = previous_id is not None and correction_of == previous_id and sequence in {None, expected_sequence}
    ...
    for trial_id, (event_id, timestamp, session_id, unit_id, score) in scores.items():
        ...
        metadata = score.get("score_metadata") if isinstance(score.get("score_metadata"), dict) else {}
...
def _summaries(observations: list[dict[str, Any]], field: str) -> list[dict[str, Any]]:
    ...
    return sorted(summaries, key=lambda item: (-item["eligible_observation_count"], str(item[field])))
...
def _session_summaries(observations: list[dict[str, Any]]) -> list[dict[str, Any]]:
    ...
    return sorted(summaries, key=lambda item: (item["last_observation_at"], item["session_ref"]), reverse=True)[:24]
...
def _input_mode_summaries(observations: list[dict[str, Any]]) -> list[dict[str, Any]]:
    ...
    for input_mode, rows in groups.items():
        ...
        summaries.append({
            ...
            "success_ratio": _round(correct / len(rows) if rows else None),
            "median_latency_ms": _round(_median(row.get("latency_ms") for row in rows), 1),
            "median_transcription_confidence": _round(_median(row.get("transcription_confidence") for row in rows)),
            "median_speech_duration_ms": _round(_median(row.get("speech_duration_ms") for row in rows), 1),
            "transcription_providers": sorted({str(row.get("transcription_provider")) for row in rows if row.get("transcription_provider")}),
        })
    return sorted(summaries, key=lambda item: (-item["eligible_observation_count"], item["input_mode"]))


def _adaptive_candidates(memory: dict[str, Any], profile_status: str, norms: HeLPNorms) -> list[dict[str, Any]]:
...
def _recovery_candidates(observations: list[dict[str, Any]], profile_status: str) -> list[dict[str, Any]]:
    if profile_status != "preliminary":
        return []
    recovery_rows = [row for row in observations if str(row.get("source") or "").startswith("hebrew_recovery_")]
    ...
    for item_id, rows in grouped.items():
        ...
        candidates.append({
            ...
            "canonical_item_id": "",
            "priority": round(priority, 2),
            "priority_components": {"personal_recovery_errors": misses * 3.0, "slow_retrieval": slow_bonus},
            "reasons": [reason for reason, enabled in (("personal_recovery_error", misses > 0), ("slow_retrieval", slow_bonus > 0)) if enabled],
...
def build_profile(
    *,
    ...
    matched_active = sum(norms.item(item.get("raw_front") or item.get("term"))["matched"] for item in active_items)
    ...
    return {
        ...
        "coverage": {
            ...
            "lexical_archive_exact_help_match_ratio": _round(matched_active / len(active_items) if active_items else None),
            ...
            "active_flashcards_exact_help_match_ratio": _round(matched_active / len(active_items) if active_items else None),
        },
        ...
        "adaptive_candidates": sorted(
            _adaptive_candidates(memory, status, norms) + _recovery_candidates(observations, status),
