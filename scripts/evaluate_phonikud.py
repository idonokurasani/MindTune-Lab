...
def manual_override(
    phonikud_ph: str, renikud_ph: str, transliteration: str, expected_stress: int
) -> str:
...
def vocal_shva_present(ph: str) -> bool:
    return (
        "ə" in ph
        or re.search(r"[ptkbdgʃʒχxʁfvθðszʨʥmnŋlrwj]e[ptkbdgʃʒχxʁfvθðszʨʥmnŋlrwj]", ph) is not None
    )
...
def main() -> int:
    ...
    for verb in pealim:
        for form_key, form in verb["forms"].items():
            ...
            phonikud_ph = phonemize(
                menukad, schema="modern", predict_stress=True, predict_vocal_shva=True
            )
            ...
            if (
                override == phonikud_ph
                and override_stress == pealim_stress
                and override_vowels == expected_vowels
            ):
                status = "verified"
