...
class ControlLoop:
    ...
    def run_session(self, frames: list[ObservationFrame]) -> ControlLoopResult:
        ...
        self.runtime.start_session(
            ...
            start_parameters={"control_loop": "clm-01", "fixture_cycle_count": len(frames)},
        )
    ...
    def _render_stimulus(
        self, control_state: MantraControlState, provenance: list[EventID], cycle_index: int
    ) -> Event:
        rendered_stimulus_id = str(make_id(RenderedStimulusID))
        media_handle = f"fixture://clm01/cycle/{cycle_index}"
        return self.runtime.emit(
    ...
    def _emit_intervention_outcome(
        ...
        return self.runtime.emit(
            ...
            {
                "outcome_id": f"outcome-{self.session_id}-{cycle_index}",
                "cognitive_state": estimate.cognitive_state.value,
                "control_state": applied_state.as_dict(),
                "cognitive_load": estimate.cognitive_load,
                "assistance_level": applied_state.assistance_level,
